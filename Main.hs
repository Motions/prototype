...
atomMoves :: [Vector3]
atomMoves = [Vector3 x y z | list@[x,y,z] <- replicateM 3 [-1,0,1], sum (map abs list) `elem` [1,2,3]]
...
intersectsChain move@(MoveBead ix delta) st = all goodNeighbor $ localNeighbors move st
    where goodNeighbor (b1@(Vector3 x1 y1 z1), b2@(Vector3 x2 y2 z2)) =
              let d = dist b1 b2 -- assume prior (not . breaksChain) check, i. e. 0 < d <= sqrt 2
              in d == 1 || (let crossPositions =
                                    case (x1 == x2, y1 == y2, z1 == z2) of
                                        (True, _, _) -> (Vector3 x1 y1 z2, Vector3 x1 y2 z1)
                                        (_, True, _) -> (Vector3 x1 y1 z2, Vector3 x2 y1 z1)
                                        (_, _, True) -> (Vector3 x1 y2 z1, Vector3 x2 y1 z1)
...
dist (Vector3 a1 a2 a3) (Vector3 b1 b2 b3) =
